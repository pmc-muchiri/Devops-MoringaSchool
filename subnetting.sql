...
c)   network ip: From our IP 156.54.1.24 we will convert to binary
	 ...
	 Network Ip has zeros  the last octet of the host portion(the first IP from the subnet)
...
d)broadcastip 
	 ...
	 from the class broadcastip is the last network so the host portion of our ip should have all 1s to be the last network broadcastip(13)
	 10011100.00110110 .00000000.00000000 --> 10011100.00110110 .11111111.11111111

	 convert to Decimal :156.54.255.255 

	 Broadcast IP has all 1s  the last octet of the host portion.
...
from this formular 2^n=Network we can say n is the bits
...
New Subnet      : 11111111.11111111.111|00000.00000000 ---> 255.255.224.0 ( 128 	64		32		16 		8 		4 		2 		1)using the third octet since others are know
																			  1      1 		 1       0      0       0       0       0 = 128+64+32 = 224
													  ---> 255.255.224.0

1. 1st Subnet:
	IP   		: 10011100.00110110|000|00001.00011000 --->156.54.1.24
